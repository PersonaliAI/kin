...
const RECOMMENDED_MODELS: Record<VoiceAgentLlmProvider, SelectOption[]> = {
  ...
  anthropic: [
    { value: "claude-3-5-haiku-latest", label: "claude-3-5-haiku-latest (Recommended)" },
...
export function VoiceAgentsView() {
  ...
  const [numberProvider, setNumberProvider] = useState<"twilio_managed" | "telnyx_managed">("twilio_managed");
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      if (editingId) {
        ...
        setBanner({ kind: "ok", text: t("created", { name: created.name }) });
      }
      setFormOpen(false);
    } catch (e) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-card border border-border p-6 rounded-2xl shadow-sm">
        ...
        <Button onClick={openCreate} className="cursor-pointer shrink-0 gap-2">
                              ...
                              <div key={call.id} className="border border-border/60 bg-card rounded-xl p-3 flex flex-col gap-1">
                                <div className="flex items-center justify-between text-xs">
                                  <span className="font-semibold">
                                    {call.direction === "inbound" ? t("inbound") : t("outbound")} — {call.from_number || call.to_number || "—"}
                                  </span>
      ...
      <Dialog
        ...
        title={editingId ? t("formDialog.editTitle") : t("formDialog.createTitle")}
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <DialogFooter>
            ...
            <Button type="submit" disabled={actionLoading} className="cursor-pointer gap-1.5">
              ...
              {editingId ? t("formDialog.save") : t("formDialog.create")}
            </Button>
          </DialogFooter>
        </form>
      </Dialog>
        ...
        <div className="space-y-4">
          <Field label={t("numberDialog.providerLabel")}>
            <Select
              ...
              onChange={(v) => setNumberProvider(v as "twilio_managed" | "telnyx_managed")}
